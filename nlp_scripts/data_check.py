'''This file performs an initial exploration of the dataframe and outputs important details, 
...
def check_duplicated_subset(data_frame, subset):
    """
    Check for duplicated values in a specific subset of the data frame and print the total.
    
    Parameters:
        - data_frame (pd.DataFrame): The input DataFrame to check for duplicates.
        - subset (list): A list of column names to check for duplicates.
    
    Returns:
        - None: This function prints the count of duplicated rows in the specified subset and does not return a value.
    """

    print("\nCount of duplicated values in subset:")
    print(data_frame.duplicated(subset=subset).sum())
...
def check(data_frame):
    ...
    check_duplicated_subset(data_frame, ['date', 'title'])

def check_fake(data_frame):
    """
    A function to call all the functions for the data exploration.
